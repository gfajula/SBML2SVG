...
public class QTArea {
	...
	private Vector<QuadTreeElement> elements = null;	
	...
	void split( int depth ) {
		if ( depth > StaticQuadTree.MAX_DEPTH|| elements==null || elements.size()<=StaticQuadTree.MAX_PER_BUCKET) return;			
		// calculate centroid
		double ax = 0;
		double ay = 0;
		...
		try {
			this.split( (int) Math.round(ax), (int) Math.round(ay) , depth+1 );
		} catch (Exception e) {
			e.printStackTrace();
			return;
		}
	...
	private void split(int x, int y, int depth ) {
//		System.out.println( "QTree depth: " + depth );
		this.x = x;
		this.y = y;
		...
		this.subAreas[0].split( depth );
		this.subAreas[1].split( depth );
		this.subAreas[2].split( depth );
		this.subAreas[3].split( depth );
	}
